...
const features = [
  ...
  { icon: Shield, title: "Secure Checkout", description: "PCI-compliant payments with fraud protection built in." },
  { icon: BarChart3, title: "Real-Time Analytics", description: "Track sales, attendance, and revenue as it happens." },
  { icon: Sparkles, title: "Fully Customizable", description: "Brand your event page to match your unique identity." },
];
  ...
  return (
    <div>
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden noise-overlay">
        <video
          className="absolute inset-0 w-full h-full object-cover opacity-80 brightness-125 contrast-110 saturate-110"
          autoPlay
          loop
          muted
          playsInline
          aria-hidden="true"
        >
          <source src={urbanpunkVid} type="video/mp4" />
        </video>
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in" style={{ animationDelay: "0.3s" }}>
            <Link to="/register"><Button size="lg" className="text-base px-8 font-semibold">Get Started Free</Button></Link>
            <Link to="/events"><Button variant="outline" size="lg" className="text-base px-8">Browse Events <ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-heading font-bold mb-4">
              Everything You Need to <span className="text-primary">Sell Out</span>
            </h2>
            <p className="text-muted-foreground max-w-xl mx-auto">Powerful tools designed for event creators who refuse to settle.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map(f => (
              <div key={f.title} className="p-6 rounded-lg border border-border bg-background hover:border-primary/30 transition-all duration-300 group">
        ...
        <section className="py-20">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between mb-10">
              ...
              <Link to="/events"><Button variant="outline" size="sm">View All <ArrowRight className="ml-2 h-3 w-3" /></Button></Link>
            </div>
      ...
      <section className="py-20 bg-card">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-heading font-bold mb-4">Ready to Launch Your Event?</h2>
          <p className="text-muted-foreground max-w-lg mx-auto mb-8">Join thousands of organizers who trust URBANPUNK to power their events.</p>
          <Link to="/register"><Button size="lg" className="text-base px-8 font-semibold animate-glow-pulse">Start Selling Tickets</Button></Link>
        </div>
      </section>
    </div>
  );
